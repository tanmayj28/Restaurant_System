...
RSpec.describe Menu, type: :model do
  context 'validation test' do 
    ...
    it 'validates menu dish name presence nil' do
      FactoryGirl.build(:menu, dish_name: nil).should_not be_valid 
    end   

    it 'validates menu dish price presence true' do
      FactoryGirl.build(:menu, dish_price: nil).should_not be_valid 
    end

    it 'validates dish description presence true' do
      FactoryGirl.build(:menu, dish_description: nil).should_not be_valid 
    end
    
    it 'validates dish_price numericality more than 0' do
      FactoryGirl.build(:menu, dish_price: -1234).should_not be_valid 
    end
    
    it 'validates dish_price numericality less than 100000' do
  ...
  context 'assosiation test' do
    ...
    it 'should belongs to customer' do
      ...
      menu = FactoryGirl.create(:menu ,customer_id:customer.id)
      menu.customer_id.should eq customer.id
    end
    ...
    it 'should not belongs to invalid customer' do
      ...
      menu = FactoryGirl.create(:menu ,customer_id:customer1.id)
      menu.customer_id.should eq customer1.id
      menu.customer_id.should_not eq customer2.id
    end
  end
